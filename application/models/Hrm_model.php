...
// New State Tax Report - Madhu
    ...
    public function citydelete_tax($citytax = null, $city) {
        ...
        if ($citytax) {
            ...
            $query = $this->db->query($sql, [$citytax, $this->session->userdata('user_id'), $city]);
        } else {
        ...
        $query3 = $this->db->query($sql3, [$this->session->userdata('user_id'), $city]);
        return true;
    }
    ...
    public function employe($emp_name = null, $date = null , $id) {
       $subquery = "(SELECT DISTINCT b.timesheet_id
                 FROM info_payslip b
                 JOIN timesheet_info a ON a.timesheet_id = b.timesheet_id
                 WHERE b.create_by = '$id'";
              if ($date) {
            $date_range = $this->parse_date_range($date);
            $dateObj    = DateTime::createFromFormat('m/d/Y', $date_range['end_date']);
            ...
            $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(a.end, ' - ', -1), '%m/%d/%Y')) = ", date("Y"));
        }
        if ($emp_name !== 'All') {
            $trimmed_emp_name = trim($emp_name);
...
// Paginated Federal income tax - Report
    public function getPaginatedfederalincometax($limit, $offset, $orderField, $orderDirection, $search, $date = null, $emp_name = 'All', $decodedId) {
        ...
        $this->db->group_by("b.timesheet_id, a.month, a.id, c.first_name, c.middle_name, c.last_name, c.employee_tax, b.f_tax, b.m_tax, b.s_tax, b.u_tax, a.cheque_date, b.templ_name, te.u_tax");
        if ($query === false) {
            return false;
        }
         $query = $this->db->get();
...
// Federal Overall Summary - Report
    public function social_tax_sumary($id, $date = null, $emp_name = 'All') {
        ...
        if ($date) {
            ...
            $this->db->where("(d.end) BETWEEN '{$date_range['start_date']}' AND '{$date_range['end_date']}'");
            $this->db->group_end();
            $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(d.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        } else {
            $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(d.end, ' - ', -1), '%m/%d/%Y')) = ", date("Y"));
        }
        $subquery = '';
        if ($emp_name !== 'All') {
            $trimmed_emp_name = trim($emp_name);
...
// Federal Overall Summary - Report
    public function social_tax_employer($id, $date = '', $emp_name = null) {
        ...
        if ($date) {
            ...
            $dateObj    = DateTime::createFromFormat('m/d/Y', $date_range['end_date']);
            $year       = $dateObj->format('Y');
            $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(ti.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
            $this->db->where('ti.end >=', $date_range['start_date']);
            $this->db->where('ti.end <=', $date_range['end_date']);
        } else {
            $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(ti.end, ' - ', -1), '%m/%d/%Y')) = ", date("Y"));
        }
...
// State Overall Tax Summary - Report - Madhu
    ...
    public function so_tax_report_employee($id, $employee_name = null, $date = null) {
    ...
    public function so_tax_report_employer($id, $emp_name = null, $date = null) {
...
// State Overall Tax Summary - Report - Madhu
    public function state_summary_employee($id, $emp_name = null, $tax_choice = null, $selectState = null, $date = null, $tax_type = null) {
        $this->db->select('DISTINCT a.timesheet_id, d.code, c.id,(b.total_amount)  ,b.net_amount , c.first_name, c.middle_name, c.last_name, d.tax_type, d.tax, (d.amount) as total_amount', false);
        $this->db->from('timesheet_info a');
        $this->db->join('info_payslip b', 'b.templ_name = a.templ_name');
        $this->db->join('employee_history c', 'c.id = b.templ_name');
        $this->db->join('tax_history_employer d', 'd.time_sheet_id = a.timesheet_id', 'left');
        ...
        $this->db->order_by('a.id', 'DESC');
        $query      = $this->db->get();
        log_message('debug', 'Final generated SQL query: ' . $this->db->last_query());
        $resultRows = $query->result_array();
        return $resultRows;
    }
...
// Individual State_tax Report
 ...
 if ($date) {
            ...
            $dateObj    = DateTime::createFromFormat('m/d/Y', $date_range['end_date']);
            $year       = $dateObj->format('Y');
            $this->db->where('d.end >=', $date_range['start_date']);
            $this->db->where('d.end <=', $date_range['end_date']);
            $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(d.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        } else {
            $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(d.end, ' - ', -1), '%m/%d/%Y')) = ", date("Y"));
        }
    ...
    public function info_for_wrf($id,$year) {
        $this->db->select('
    ...
    public function total_amt_wr30($id,$year) {
        $this->db->select('SUM(c.total_amount) AS OverallTotal, COUNT(*) AS count,COUNT(DISTINCT a.templ_name) AS emp_count');
        $this->db->from('timesheet_info a');
        $this->db->join('employee_history b', 'b.id = a.templ_name', 'left');
        $this->db->join('info_payslip c', 'c.timesheet_id = a.timesheet_id', 'left');
        $this->db->where("b.payroll_type NOT LIKE 'Sales Partner'");
 ...
 public function f940_excess_emp()
{
  ...
  public function getQuarterlyMonthData($id,$quarter,$year)
{
      $currentYear = date('Y');
    switch ($quarter) {
        case 'Q1':
  ...
  public function fetchQuarterlyData($user_id, $quarter)
{
    ...
    public function info_info_for_salescommssion_data($user_id, $quarter, $year) {
    ...
    public function info_for_nj($user_id, $quarter ,$year) {
    ...
    public function sum_of_tax_amount($user_id,$year) {
        ...
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(timesheet_info.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $row = $query->row();
            return $row->amount;
    ...
    public function get_sc_info($user_id) {
        ...
        $this->db->from('info_payslip a');
        $this->db->join('employee_history b', 'b.id = a.templ_name');
        $this->db->join('timesheet_info c', 'c.templ_name = b.id');
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(c.end, ' - ', -1), '%m/%d/%Y')) = ", date("Y"));
    ...
    public function getoveralltaxdata($id, $year) {
    ...
    public function w2get_payslip_info($id, $year) {

        $this->db->select('SUM(a.total_amount) as overalltotal_amount ,SUM(a.f_tax) as  ftotal_amount ,SUM(a.s_tax) as  stotal_amount , SUM(a.m_tax) as  mtotal_amount');
        $this->db->from('info_payslip a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.timesheet_id');
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->where('a.templ_name', $id);
        $this->db->where('a.tax IS NOT NULL');
    ...
    public function getother_tax($id, $year) {

        $this->db->select('a.code,a.tax, SUM(a.amount) AS amount');
        $this->db->from('tax_history a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.time_sheet_id');
        $this->db->where('a.employee_id', $id);
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->where("(a.tax_type = 'other_tax' OR a.tax_type = 'other_working_tax')");
        $this->db->group_by('a.code,a.tax');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function w2total_state_tax($id, $year) {
        $this->db->select('ROUND(SUM(a.amount), 2) as overalltotal_statetax');
        $this->db->from('tax_history a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.time_sheet_id');
        $this->db->where('a.employee_id', $id);
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->where_in('a.tax_type', ['state_tax']);
        $this->db->where_in('a.tax', ['Income tax']);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function w2totalstatetaxworking($id, $year) {
        $this->db->select('ROUND(SUM(a.amount), 2) as overalltotal_statetaxworking');
        $this->db->from('tax_history a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.time_sheet_id');
        $this->db->where('a.employee_id', $id);
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->where_in('a.tax_type', ['living_state_tax']);
        $this->db->where("a.tax LIKE '%Income tax%'");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function getcounty_tax($id, $year) {
        ...
        $this->db->where('a.employee_id', $id);
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->where("(a.tax_type = 'living_county_tax' OR a.tax_type = 'working_county_tax')");
        $this->db->group_by('a.code,a.tax');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
    ...
    public function w2total_local_tax($id, $year) {

        $this->db->select('a.code, ROUND(SUM(a.amount), 2) as overalltotal_localtax');
        $this->db->from('tax_history a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.time_sheet_id');
        $this->db->where('a.employee_id', $id);
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->where_in('a.tax_type', ['local_tax']);
    ...
    public function w2total_livinglocaldata($id, $year) {

        $this->db->select('ROUND(SUM(a.amount), 2) as livinglocaltax');
        $this->db->from('tax_history a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.time_sheet_id');
        $this->db->where('a.employee_id', $id);
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->where_in('a.tax_type', ['living_local_tax']);
        $this->db->group_by('a.code, a.tax_type');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function gettaxother_info($id, $year) {

        $this->db->select('a.code,a.tax, SUM(a.amount) as amount');
        $this->db->from('tax_history a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.time_sheet_id');
        $this->db->where('a.employee_id', $id);
        $this->db->where("(a.tax_type = 'state_tax' OR a.tax_type = 'living_state_tax' OR a.tax_type = 'other_tax' OR a.tax_type = 'other_working_tax')");
        $this->db->where("a.tax NOT LIKE '%Income Tax%'");
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $this->db->group_by('a.code,a.tax');
        $this->db->order_by('a.tax');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function get_payslip_info($id, $year) {
        $this->db->select('SUM(a.total_amount) as overalltotal_amount , SUM(a.f_tax) as  ftotal_amount , SUM(a.s_tax) as  stotal_amount ,  SUM(a.m_tax) as  mtotal_amount , a.sales_c_amount');
        $this->db->from('info_payslip a');
        $this->db->join('timesheet_info b', 'b.timesheet_id = a.timesheet_id');
        $this->db->where('a.tax IS NOT NULL');
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(b.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
    ...
    public function sum_of_country_tax($end_date = null, $empid, $timesheetid, $user_id, $year) {
        $this->db->select('
...
// Retrieve Federal Tax - Madhu
    public function federal_tax_info($tax_type, $employee_status, $final, $federal_range, $user_id, $year) {
    ...
    public function countydelete_tax($countytax = null, $county) {
        ...
        if ($countytax) {
            ...
            $query = $this->db->query($sql, [$countytax, $this->session->userdata('user_id'), $county]);
        } else {
        ...
        $query3 = $this->db->query($sql3, [$this->session->userdata('user_id'), $county]);
        return true;
    }
...
// Timesheet Details - Madhu
    ...
    public function delete_tax($tax = null, $state) {
        ...
        if ($tax) {
            ...
            $query = $this->db->query($sql, [$tax, $this->session->userdata('user_id'), $state]);
        } else {
        ...
        $query3 = $this->db->query($sql3, [$this->session->userdata('user_id'), $state]);
        return true;
    }
...
//Used in f940
    public function get_paytotal($id,$year) {
        $this->db->select('SUM(b.total_amount) as total_grosspay');
        $this->db->from('info_payslip b');
        $this->db->join('timesheet_info a', 'a.timesheet_id = b.timesheet_id');
        $this->db->where('b.tax IS NOT NULL');
        $this->db->where('b.create_by', $id);
        $this->db->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(a.end, ' - ', -1), '%m/%d/%Y')) = ",  $year);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
...
// To get the state tax details - Used in payslip and time_list functions
    public function get_state_details($find, $table, $where, $state, $user_id, $year = null) {
        $this->db->select($find)->from($table)->where($where, $state)->where('created_by', $user_id);
        if ($table != 'state_and_tax' && ! empty($year)) {
            $this->db->where('year', $year);
        }
        $query = $this->db->get();
        if ($query === false) {
...
// To get the state tax details - Used in state_tax function
    public function working_state_tax($taxname, $employee_status, $final, $local_tax_range, $stateTax = "", $user_id, $payroll, $payroll_frequency, $year) {
...
//To get the cumulative state tax amount of specific employee
    public function get_cumulative_tax_amount($tax, $end, $employee, $tax_type, $year) {
        $this->db->select('SUM(tax_history.amount) as total_amount')
            ...
            ->where("YEAR(STR_TO_DATE(SUBSTRING_INDEX(timesheet_info.end, ' - ', -1), '%m/%d/%Y')) = ", $year);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
...
// All Federal Taxes - Madhu
    public function allFederaltaxes($taxType, $user_id, $year) {
        $this->db->select('*');
        $this->db->from('federal_tax');
...
// Delete City and County tax - Payroll Setting 
    public function delete_payrolldata($id, $tax_type, $year) {
        $this->db->where('id', $id);
        $this->db->where('year', $year);
        if ($tax_type == 'citytax') {
